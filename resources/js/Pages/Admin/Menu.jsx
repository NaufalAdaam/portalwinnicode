...
import {
    ...
    X,
    GripVertical,
    Edit2, // <-- Tambahkan Edit2
    Trash2, // <-- Tambahkan Trash2
    TrendingUp,
    Clock
} from "lucide-react";
                ...
                <div className="pt-2 space-y-1 ml-5">
                    {subLinks.map((link) => (
                        <SidebarLink
                            ...
                            href={link.href}
                            icon={link.icon}
                            isActive={link.active} // <-- Meneruskan status aktif
...
function DraggableTableRow({
    ...
    return (
        <tr
            ...
            onDragEnd={onDragEnd} // <-- Menambahkan onDragEnd
        ...
        >
            ...
            {/* Kolom Posisi --- STYLING BIRU DIHILANGKAN */}
            <td className="px-6 py-4 text-gray-600 dark:text-gray-300">
                {menu.urutan}
            </td>
            {/* Kolom Action */}
...
export default function MenuLayout() {
    ...
    // State dragIndex DITAMBAHKAN KEMBALI
    ...
    const fetchMenus = async () => {
        setLoading(true);
        try {
            const res = await axios.get("/api/footer-menu");
    ...
    const handleCloseModal = () => {
        setShowModal(false);
        resetForm(); // Selalu reset form saat ditutup
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const data = {
            ...
            url: formData.url,
            urutan: formData.urutan || menus.length + 1, // Fallback jika urutan kosong
        };

        try {
            if (editingId) {
                await axios.put(`/api/footer-menu/${editingId}`, data);
            } else {
                await axios.post("/api/footer-menu", data);
            }
            handleCloseModal();
            fetchMenus(); // Fetch ulang data untuk mendapatkan list terbaru
        } catch (err) {
    ...
    const handleDelete = async (id) => {
        if (confirm("Yakin ingin menghapus menu ini?")) {
            try {
                await axios.delete(`/api/footer-menu/${id}`);
                fetchMenus(); // Fetch ulang data
            } catch (error) {
    ...
    // --- Drag and Drop Handlers (DITAMBAHKAN KEMBALI) ---
    ...
    const handleDragEnd = () => {
        setDragIndex(null); // Reset dragIndex saat selesai (baik drop atau cancel)
    };
    ...
    const handleDrop = async (e, dropIndex) => {
        ...
        try {
            ...
            await axios.put("/api/footer-menu/reorder", { menus: reorderData });
        } catch (error) {
            console.error("Gagal mengupdate urutan menu:", error);
            // Rollback (ambil data baru dari server) jika gagal
            fetchMenus();
        }
    };
    ...
    // --- Render JSX ---
    return (
        <AdminLayout title="Layout / Menu Bawah" activeLinkTitle="Menu Bawah">
            {/* Konten Halaman Spesifik */}
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                <h2 className="text-lg font-semibold mb-2 text-gray-900 dark:text-gray-100">
                    Layout / Menu Bawah
                </h2>
                {/* Toolbar (Tombol Add) --- TEKS TOTAL DIHILANGKAN */}
                            ...
                            {loading ? (
                                ...
                                menus.map((menu, index) => (
                                    <DraggableTableRow
                                        ...
                                        menu={menu}
                                        index={index}
                                        onEdit={handleOpenModal} // Menggunakan handleOpenModal
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    min="1"
                                />
                                {/* TEKS BANTUAN DI MODAL DIHILANGKAN */}
                            </div>
